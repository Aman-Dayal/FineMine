...
# from OAuth import authorize_redirect
...
try:
    ...
    db = mongo.get_database('Fine_Mine_2_0')
    collection1 = db.get_collection('binuser')
...
def getdata():
    if request.method == 'POST': 
        ...
        if request.form['Get_data']:     
            ...
            check = collection1.find_one(filter=dict(contact_no=request.form['contact']))            
        # [('default', '                 '),
        #  ('AS', 'All Skills'), 
...
def getall():
    if request.method == 'POST': 
            ...
            if request.form['Get_all']:               
                result = collection1.find({},{'_id':0,'candidate_name':1, y:1})
                result1 = []
                # print(result)
                for s in result:
                    print(s) 
...
def downloadFile ():
    # i =var 
    data = collection1.find_one(filter=dict(contact_no="07019328368"))
    ...
    if request.method == 'POST':        
        ...
        if request.form['submit']:        
            ...
            if filename != '':
                ...
                # print(check)
                if check == None:
                    dbResponse = collection1.insert_one(newdata)
